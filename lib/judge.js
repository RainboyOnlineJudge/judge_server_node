    ...
    return compile(code_path,compile_out,language) //第一步 编译
            ...
            .then(function(){   //评测
                return Promise.each(input_list,function(item,index){
                    ...
                    exec_output_list[index] = tmpConfig['output_path'] = join(out_path,index+'.out');
                    var t_result ={}
                    return _judger(tmpConfig)
                        .then(function(result){ //对结果进行分析
                            judgerResult[index] = result;
                        })
                },{concurrency:5});
            })
            .then(function(){ //
                return Promise.each(judgerResult,function(result,index){
                    var t_result= {};
                    ...
                    t_result.real_time = result.result.real_time;
                    if (t_result.result == 0 ){ //diff
                        return diff(output_list[index],exec_output_list[index])
