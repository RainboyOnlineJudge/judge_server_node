...
//describe('文件上传测试',function(){
    ...
    //it('zip上传',function(done){
        //postFile(uid,join(n_path,'test.zip'),'zip','http://127.0.0.1:'+config.port+'/file')
    ...
    //it('代码文件上传',function(done){
        //var codepath =join(n_path,'test.cpp');
        //var type = ext(codepath)
        //postFile(uid,codepath,type,'http://127.0.0.1:'+config.port+'/file')
            //.then(function(result){
                //result.state.should.eql('success');
                //result.uid.should.eql(uid);
                //var out = out_cotent(join(n_path,'test.cpp'));
                //out.should.eql( out_cotent( join(config.prefix,uid,'main.cpp')));
                //done();
            //})
    //})
//})
